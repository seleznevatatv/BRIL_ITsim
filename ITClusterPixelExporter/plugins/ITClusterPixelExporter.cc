...
struct ITModuleEvent
{
    ...
    void print()
    {
        ...
        for (auto cluit : this->clusters)
        {
            ...
            for (auto hitit : cluit)
                std::cout << "(" << (hitit >> 16) << "," << (hitit & 0xFFFF) << ") ";
            std::cout << std::endl;
            clucount++;
        }
    }
...
//
ITClusterPixelExporter::ITClusterPixelExporter(const edm::ParameterSet &iConfig) : tgeomHandle(esConsumes()), tTopoHandle(esConsumes()), m_tokenClusters(consumes<edmNew::DetSetVector<SiPixelCluster>>(iConfig.getParameter<edm::InputTag>("clusters")))
                                                                                   ...
                                                                                   ,
                                                                                   m_tokenSimLinks(consumes<edm::DetSetVector<PixelDigiSimLink>>(iConfig.getParameter<edm::InputTag>("simlinks"))), m_Eventoffset(iConfig.getParameter<int>("eventNoOffset"))
{
    ...
    for (typename edm::DetSetVector<PixelDigi>::const_iterator DSVit = digis->begin(); DSVit != digis->end(); DSVit++)
    {
        ...
        if (mType == TrackerGeometry::ModuleType::Ph2PXF && detId.subdetId() == PixelSubdetector::PixelEndcap)
        {
            ...
            if (disk > 8)
            {
                ...
                for(auto cluvec : m_event.clusters)
                {
                    for(auto clu : cluvec) {
